...
function BackdropWord({ children, blue = false, large = false, x = 0, y = 0, scale = 1 }: { children: React.ReactNode; blue?: boolean; large?: boolean; x?: any; y?: any; scale?: any }) {
  return <motion.p aria-hidden="true" style={{ x, y, scale, WebkitTextStroke: `1px ${blue ? "rgba(7, 88, 200, .7)" : "rgba(255, 255, 255, .7)"}`, color: "transparent" }} className={`absolute right-[3vw] whitespace-nowrap text-right font-serif leading-none tracking-[-.1em] ${blue ? "opacity-25" : "opacity-20"} ${large ? "top-[20%] text-[clamp(6rem,24vw,24rem)]" : "top-[27%] text-[clamp(4.5rem,18vw,18rem)]"}`}>
    {children}
  </motion.p>;
}
...
function ScrollCue({ blue = false }: { blue?: boolean }) {
  ...
  return <div aria-hidden="true" className={`absolute bottom-28 left-1/2 z-10 flex -translate-x-1/2 items-start gap-3 sm:bottom-9 ${blue ? "text-[#0758c8]" : "text-white"}`}>
  ...
  return <div className="bg-[#0758c8] text-white">
    <main className="relative" style={{ height: "900vh" }}>
      <div className="sticky top-0 h-[100svh] overflow-hidden">
        ...
        <Scene opacity={s1} className="items-center px-5 pt-20 sm:px-[6vw]"><BackdropWord large x={f1.wordX} scale={f1.wordScale}>MOVE</BackdropWord><motion.img src={womanFigure} alt="" aria-hidden="true" loading="eager" decoding="async" onLoad={(event) => event.currentTarget.decode().catch(() => undefined).finally(() => setWomanReady(true))} style={{ x: womanX, y: womanY, scale: womanScale, opacity: womanReady ? 1 : 0, transformOrigin: "bottom center" }} className="absolute bottom-[5vh] right-[-8vw] z-[5] h-[72svh] w-auto object-contain object-bottom transition-opacity duration-300 md:bottom-0 md:right-[18vw] md:h-[94svh]" /><CinematicCopy {...f1} className="-translate-y-[4vh]"><p className="font-mono text-[9px] tracking-[.3em]">01 / MOVE</p><h1 className="-ml-2 mt-10 max-w-6xl font-serif text-[6.5rem] leading-[.82] tracking-[-.12em] sm:-ml-[4vw] sm:text-[clamp(5rem,16vw,16rem)]">運ぶ。</h1></CinematicCopy><ScrollCue /></Scene>
        ...
        <Scene opacity={s5} className="items-center px-5 sm:px-[6vw]"><BackdropWord x={f5.wordX} y="-5vh" scale={f5.wordScale}>CALL</BackdropWord><motion.img src={truckFigure} alt="" aria-hidden="true" style={{ x: truckX, y: truckY, scale: truckScale, transformOrigin: "bottom right" }} className="absolute bottom-[12vh] right-[-28vw] z-[5] h-[44svh] w-auto object-contain object-bottom md:bottom-[1vh] md:right-[-2vw] md:h-[66svh]" /><CinematicCopy {...f5} className="-translate-y-[3vh]"><p className="font-mono text-[9px] tracking-[.3em]">05 / CALL NOW</p><h2 className="-ml-2 mt-10 font-serif text-[4rem] leading-[.95] tracking-[-.11em] sm:hidden">今すぐ<br />相談する。</h2><h2 className="mt-10 hidden whitespace-nowrap font-serif leading-none tracking-[-.11em] sm:-ml-[4vw] sm:block sm:text-[clamp(3.4rem,12vw,12rem)]">今すぐ相談する。</h2></CinematicCopy><div className="pointer-events-auto absolute bottom-7 left-1/2 z-10 -translate-x-1/2 sm:bottom-9 sm:left-[6vw] sm:translate-x-0"><motion.div style={{ opacity: finalCtaOpacity, y: finalCtaY }} className="scale-110"><PhoneAction location="final_scene" /></motion.div></div></Scene>
      </div>
    </main>
